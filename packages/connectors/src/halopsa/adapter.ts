...
const haloListOpenTicketsTool: ConnectorToolDefinition<z.infer<typeof haloTicketFiltersSchema>, NormalizedToolResponse> = {
  ...
  description:
    "Use when the user wants a queue-style view of active tickets, open incidents, or tickets for a customer. Also use for counting tickets, reporting on ticket volumes, exporting tickets, or querying tickets by date range (e.g. 'last quarter', 'this month', 'last 30 days', 'Q1 2025', 'YTD'). Returns up to 250 tickets in a single call — the server handles HaloPSA pagination internally, so do NOT call this tool repeatedly for the same query and do NOT pass page_no / page_size / paginate; they are ignored and only there for back-compat. For large exports or counting/historical queries, pass the full natural language question as the query parameter (the server will automatically scale to the full 250-ticket window). Supports natural language date ranges, text search across summaries and categories, client_id, site, status arrays, category filters (category_1 through category_4 as numeric ID arrays — call list_halo_categories first to discover category IDs), ordering, and search fields.",
